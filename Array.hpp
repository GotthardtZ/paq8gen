#ifndef PAQ8GEN_ARRAY_HPP
#define PAQ8GEN_ARRAY_HPP
...
public:
    ...
    ALWAYS_INLINE T &operator[](uint64_t i) {
      CHECK_INDEX(i, usedSize);
      return data[i];
    }

    ALWAYS_INLINE const T &operator[](uint64_t i) const {
      CHECK_INDEX(i, usedSize);
      return data[i];
    }
